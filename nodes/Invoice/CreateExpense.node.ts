...
export class CreateExpense implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'PEAK Create Expense',
		name: 'createExpense',
		icon: 'file:../../assets/PEAK.svg',
		group: ['output'],
		version: 1,
		subtitle: '={{$parameter["operation"]}}',
		description: 'Send JSON payload to create expense via PEAK API',
		defaults: {
			name: 'Create Expense',
		},
		inputs: ['main'],
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		let endpointUrl = 'https://peakengineapidev.azurewebsites.net/api/v1/expenses';
