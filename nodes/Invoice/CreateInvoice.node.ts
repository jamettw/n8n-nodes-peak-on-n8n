...
export class CreateInvoice implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'PEAK Create Invoice',
		name: 'createInvoice',
		icon: 'file:../../assets/PEAK.svg',
		group: ['output'],
		version: 1,
		subtitle: '={{$parameter["operation"]}}',
		description: 'Send JSON payload to create invoice via PEAK API',
		defaults: {
			name: 'Create Invoice',
		},
		inputs: ['main'],
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		let endpointUrl = 'https://peakengineapidev.azurewebsites.net/api/v1/invoices';
